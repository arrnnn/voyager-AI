...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .pro-badge { display: inline-block; background: linear-gradient(135deg, #2563eb, #7c3aed); color: white; font-size: 11px; padding: 2px 8px; border-radius: 10px; margin-left: 8px; }
...
<div class="cover">
  ...
  <div class="cover-content">
    ...
    <p>Your Personalised AI Travel Plan by Voyager AI${plan === 'pro' ? ' <span class="pro-badge">PRO</span>' : ''}</p>
    <div class="pills">
      <span class="pill">📅 ${trip.duration} days</span>
      <span class="pill">💰 $${trip.budget} / Rs.${(trip.budget * 83).toLocaleString('en-IN')}</span>
  ...
  <div class="budget-grid">
    ${Object.entries(budget).map(([k, v]) => `
      <div class="budget-card">
        ...
        <div class="inr">Rs.${(v * 83).toLocaleString('en-IN')}</div>
      </div>
    `).join('')}
  </div>
</div>` : ''}
  ...
  <div class="hotel-grid">
    ${hotels.map(h => `
      <div class="hotel">
        ...
        <div class="hotel-price">$${h.price}/night / Rs.${(h.price * 83).toLocaleString('en-IN')}</div>
...
<div class="footer">
  Generated by Voyager AI${plan === 'pro' ? ' Pro' : ''} • ${new Date().toLocaleDateString()} • Open in browser and press Ctrl+P to save as PDF
</div>
